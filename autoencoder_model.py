...
class MMAE(nn.Module):
    ...
    def forward(self, omics_1, omics_2):
        ...
        latent_data = torch.mul(encoded_omics_1, self.a) + torch.mul(encoded_omics_2, self.b)
    ...
    def train_MMAE(self, train_loader, learning_rate=0.001, device=torch.device('cpu'), epochs=500):
        ...
        for epoch in range(epochs):
            ...
            for (x,y) in train_loader:
                omics_1 = x[:, :self.in_feas[0]]    # 选择所有的行，self.in_feas[0]从第一列到第self.in_feas[0]-1列。
                omics_2 = x[:, self.in_feas[0]:self.in_feas[0]+self.in_feas[1]] # 选择所有的行，从第 self.in_feas[0]列到第 self.in_feas[0] + self.in_feas[1] - 1 列的所有列的数据。
        ...
        plt.xlabel('epochs')
        plt.ylabel('loss')
        plt.savefig('result/AE_train_loss_T2D.png')